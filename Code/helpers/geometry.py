...
def find_intersect(ray, wall):
    """Find the intersection between a ray and a wall (two 2-D line segments), if it exists."""
    w_x1, w_y1 = wall.x1, wall.y1
    w_x2, w_y2 = wall.x2, wall.y2

    r_x1, r_y1 = ray.x1, ray.y1
    r_x2 = r_x1 + ray.length * math.cos(ray.alloc_angle)
    r_y2 = r_y1 + ray.length * math.sin(ray.alloc_angle)
