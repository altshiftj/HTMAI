...
def print_cells_csv():
    path_L23_active =   'C:\Environments\HTMAI Output\L23_Active'
    path_L4_active =    'C:\Environments\HTMAI Output\L4_Active'
    path_L6a_active =    'C:\Environments\HTMAI Output\L6a_Active'
    ...
    for i in range(256*32):
        ...
        if not df_1.loc[df_1['cell'] == j].empty:
            df_1.loc[(df_1['cell'] == j)].to_csv(f'C:\Environments\HTMAI Output\L23_Active\cell{j}.csv')

    for i in range(256*32):
        j = i
        if not df_2.loc[df_2['cell'] == j].empty:
            df_2.loc[(df_2['cell'] == j)].to_csv(f'C:\Environments\HTMAI Output\L4_Active\cell{j}.csv')

    for i in range(256*32):
        j = i
        if not df_3.loc[df_3['cell'] == j].empty:
            df_3.loc[(df_3['cell'] == j)].to_csv(f'C:\Environments\HTMAI Output\L6a_Active\cell{j}.csv')